...
── Build modes ──────────────────────────────────────────────────────────────
Controlled by atom_agent.yaml ci.provider (overridable with --ci):

  local   docker build on this machine, then push and submit (default)
  gitlab  trigger a GitLab CI pipeline, poll until complete, then submit
  github  not yet supported

── GitLab CI flow (ci.provider: gitlab) ─────────────────────────────────────
1. Reads ci.origin (default: gl_origin) to get the GitLab repo URL
2. Resolves PAT from: ~/.atom/credentials → GITLAB_TOKEN env → interactive prompt
   First-run prompt saves the token to ~/.atom/credentials (chmod 600)
3. Warns if sdk_image and agent repo are in different GitLab groups
   (cross-group pull requires a group deploy token — see docs/CI_BUILD.md)
4. Triggers pipeline with: ATOM_BUILD=true, ATOM_IMAGE_TAG=<git-sha>, SDK_IMAGE=<sdk_image>
5. Polls every 30 s until success or failure (up to 30 min)
6. On success: submits image ref to atom-studio as a pending deployment
7. On failure: prints the failed pipeline URL for log access

── atom_agent.yaml ci block ─────────────────────────────────────────────────
Generated by atom create. Fill in agent_id and domain_id from atom-studio.

  ci:
    provider: gitlab       # local | gitlab
    origin: gl_origin      # git remote pointing to the agent's GitLab repo
    branch: main
...
Examples:
  ...
  atom deploy --ci local --message "hotfix"    # force local build
  atom deploy --ci gitlab --token glpat-...    # GitLab CI, explicit PAT`,
	RunE: runDeploy,
}
