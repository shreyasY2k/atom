...
## Section 10 — Domain/Subdomain & Compliance Report (3 min)

Navigate to **Agents → Registry**

> "Notice the filter bar at the top. Agents and tools are tagged with domain and subdomain — the same taxonomy that appears in the spec YAML. Clicking 'banking' shows only banking agents. Clicking 'general' shows only general agents."

**Demonstrate domain filter:**
1. Click the **banking** chip → shows only `kyc-refresh`
2. Click the **general** chip → shows only `customer-qa-agent`
3. Click **All** to reset

Navigate to **Tools → Tool Registry**

> "Same pattern for tools. Tools are grouped by domain. The banking group has 10 tools — KYC, fraud, treasury, securities. Click any accordion to expand."

**Demonstrate compliance report:**
1. Click on **customer-qa-agent** → open the **Compliance** tab
2. Select period: **Last 30 days**
3. Click **Generate Report**
4. Wait 20–30 seconds for the report to complete

> "This is a formal compliance report generated by Gemini 3.1 Pro from the agent's actual audit trail — LLM call events, guardrail blocks, PII redactions, session activity. It reads exactly like a document you would file with compliance."

Point to the report sections:
- Executive Summary
- Security Posture (10 blocks, 16 PII redactions in this session)
- Data Handling (EMAIL, SSN, CC detected and masked)
- Audit Chain Integrity (HMAC-signed events in MinIO)
- Risk Assessment + Recommendations

> "It's downloadable as Markdown. Reproducible. And because it reads from the same immutable audit trail, it's consistent across every run."
...
> 2. Tagged it with domain and subdomain — filterable alongside all other platform agents
...
> 5. Blocked a prompt injection before it touched the LLM — 10-layer guardrail, L1 fail-closed
> 6. Generated a compliance report directly from the audit trail
> 7. Verified the tamper-evident audit chain
>
...
| Agent Registry | http://localhost:5173/agents |
| Tool Registry | http://localhost:5173/tools |
| Security Command Center | http://localhost:5173/command-center |
...
| Domain filter shows no results | Expected if no agents have that domain. Deploy a spec with matching `metadata.domain` |
| Second message in session blocked after injection | Start a new session. Platform strips blocked turns from history automatically |
| Compliance report stuck at `generating` | `docker compose logs builder-backend --tail 20` — check LiteLLM call |
| LiteLLM 401 on `/health` | Use `/health/liveliness` instead. The 401 is from the admin UI, not a real error |
