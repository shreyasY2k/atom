...
>
> Platform is rebranded `atom` (was `atom-agent-platform`). Rebranding
...
`actor_id: user:demo@atom.io` is updated to read from the request
...
1. `atom-agent-platform` → `atom-agent-platform` (or just `atom`
...
2. `atom` → `atom` in:
   ...
   - `cli/atom.py` → rename to `cli/atom.py`; update setup.py
...
- platform the company itself, where mentioned (e.g. "platform BFSI
  team", "platform sales") — that's the parent; the platform is `atom`
- The legal phrase "platform Agent Platform" if used in any
  marketing/leave-behind copy — that's a product naming question for
  platform branding, not for this technical rebrand

### F.3 Verify after sweep

- `grep -ri "atom" .` returns nothing in code/docs
- `docker compose up` works with renamed containers
- CLI invocable as `atom` (not `atom`)
...
Already partially started (you completed the platform rebrand). Add a section
...
- [ ] Repo rebranded from `atom` to `atom` across all targets in F.1
- [ ] `grep -ri "atom" .` returns no hits in code/docs (excluding
      legitimate references to platform the company)
