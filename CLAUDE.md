# CLAUDE.md — Atom Agent Platform

> **Read this first, every session.** Defines what we're building, the constraints, and where to find the rest.

## Mission

Build a production platform with **two surfaces**:

1. **Agent Builder** — produces audited, deployable agents from a versioned spec.
2. **Workflow Composer** — loads existing business processes as graphs; specific human-decision nodes are replaced with agents; humans retained at decision boundaries.
...
The flagship use case is **Asset Transfer Service (ATS)** — an existing 9-step workflow where two routine human steps are replaced with agents.

The platform must be:

- Domain-credible (real process shapes, not toy examples)
...
1. **Every LLM call goes through GATE:8083 first**, then to LiteLLM at `http://litellm:4000`. Agents use `LITELLM_BASE_URL=http://gate:8083`. GATE:8083 is the mandatory audit chokepoint for all LLM traffic.
...
9. **Audit logs go to MinIO with object lock**, 90-day compliance retention.
10. **Humans retain final accept/override.** Workflows must have a `human_task` node before any state-changing external call (or immediately after if the call is reversible).
11. **Synthetic data only.** All external system calls hit mock services.
12. **Build from source** for AgentScope, AgentScope Runtime, AgentScope Studio, ReMe. Temporal can use the official image (CNCF, not vendor-branded).
...
|---|---|---|---|
| **A. Visual + AI** | Builder UI generates spec + skill from prose | Composer UI, drag-and-drop nodes, agents picked from registry | Fast prototyping; primary path |
| **B. CLI scaffold + manual** | `atom agent scaffold <name>` produces stub spec + stub skill in repo; developer fills in | Composer UI or `atom workflow init` produces stub `workflow-spec.yaml` | Realistic dev workflow; secondary path |
| **C. Full natural-language** | Same as A | Composer also generates from prose | Natural-language workflow generation; off the critical path. Disable cleanly if it misbehaves. |
...
- End-to-end: 2 nodes in ATS replaced with agents, workflow runs with both routine and high-value paths
...
- HiClaw, A2A registry
- Real user auth — hardcoded default user (IDP integration is Phase 2)
- Multi-tenancy
- K8s deployment — Phase 2+
- Insurance OCR use case — kept in codebase as a skill library breadth example, not in active scope
...
| Decision | Why |
|---|---|
| Temporal over n8n / Camunda / build-from-scratch | Enterprise deployments exist, handles long-running workflows with human gates, hire-able engineers, audit story is solid. |
| Drop OCR insurance from active scope | Two depth use cases (treasury + ATS workflow) is enough. OCR stays in code as breadth. |
...
| Service-account identity at deploy time | Enterprise audit requires non-human identities (NHIs) tracked separately from humans. Required for SOC 2 / ISO 27001 compliance. |
| ATS as flagship workflow | Real process with human-replaceable steps — clean demonstration of time compression + audit unification. |
...
| `reasoning_mode` field per agent (prescribed \| guided) | Makes the prescribed-vs-guided trade-off an explicit, auditable spec field rather than implicit in role prose. Prescribed = auditable deterministic path. Guided = flexible reasoning for less-structured tasks. Validators can enforce mode-appropriate patterns. |
...
| Studio reused as engineer surface, not embedded in main UI | Studio's chrome (project list, model config) is aimed at engineers. We reimplement the UX patterns (chat bubbles, collapsible traces) in our own UI and link to Studio for engineers. Not iframed — avoids CORS complexity and keeps the platform UI clean. |
